...
let confirmedLabels = [null, null, null]; // Store the confirmed labels
...
// Check if all labels are generated and show review section
...
// Confirm the labels and store them in the array
function confirmLabels() {
    ...
    for (let i = 0; i < 3; i++) {
        ...
        confirmedLabels.push(label || null);
    }
    
    console.log("Confirmed labels:", confirmedLabels);
    
    // You can now use confirmedLabels array - for example, pass to another page:
    // window.location.href = `next-page.html?labels=${encodeURIComponent(JSON.stringify(confirmedLabels))}`;
    
    // Or store in localStorage:
    localStorage.setItem('doodletunes_labels', JSON.stringify(confirmedLabels));
    
    // Show success message - CAN BE REMOVED AFTER INTEGRATION
    alert(`Labels confirmed!\n1. ${confirmedLabels[0] || 'empty'}\n2. ${confirmedLabels[1] || 'empty'}\n3. ${confirmedLabels[2] || 'empty'}\n\nStored in localStorage as 'doodletunes_labels'`);
}
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    if (savedLabels) {
        try {
            confirmedLabels = JSON.parse(savedLabels);
            console.log("Loaded saved labels:", confirmedLabels);
        } catch (e) {
