...
class LyricGenerator:
    ...
    def _create_prompt(self, request: LyricRequest) -> str:
        ...
        You are trying to generate lyrics for a Billboard Top 100 Hit song. 
        Write a short song verse with exactly {request.num_lines} lines about {labels_str}.
Requirements:
- The lyrics should be about: {labels_str}
- Emotion/mood: {request.emotion}
- Exactly {request.num_lines} lines
- Make it easy to sing, easy to understand, and easy to remember
- Use simple vocabulary and grammar, avoid end rhymes, and make it catchy and memorable
- Reference the objects ({labels_str}) naturally
- Match the {request.emotion} mood
Write only the lyrics, one line per line, no numbering or labels:"""
    ...
    def _call_huggingface(self, prompt: str) -> Optional[str]:
        ...
        if not api_key:
            print("No HF_TOKEN/HUGGINGFACE_API_KEY set; skipping Hugging Face and using fallback/provider backup.")
            return None
    ...
    def _clean_lyrics(self, text: str) -> str:
        ...
        for line in lines:
            line = line.strip()
            if line and not line.startswith(('Write', 'Requirements', 'The lyrics', 'Emotion')):
